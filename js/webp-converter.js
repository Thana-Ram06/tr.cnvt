...
uploadArea.addEventListener('drop', (e) => {
    ...
    if (files.length > 0 && files[0].type === 'image/webp') {
...
function handleFileSelect(file) {
    if (file.type !== 'image/webp') {
        showError('Please select a valid WEBP file.');
        return;
    }
...
convertBtn.addEventListener('click', () => {
    ...
    try {
        ...
        img.onload = () => {
            ...
            if (format === 'jpg') {
                // Fill background color first (for transparent WEBP)
            ...
            canvas.toBlob((blob) => {
                ...
                link.href = url;
                link.download = selectedFile.name.replace(/\.webp$/i, `.${format}`);
                link.click();
