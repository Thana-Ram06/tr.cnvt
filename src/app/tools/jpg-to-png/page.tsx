...
export default function JpgToPngPage() {
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (f?.type === 'image/jpeg') {
      ...
      setStatus('idle');
      setErrorMsg('');
    } else if (f) setErrorMsg('Please select a JPG file.');
  }, []);
  ...
  const handleConvert = useCallback(async () => {
    ...
    setErrorMsg('');
    try {
      await convertImage(file, 'image/png', 'png');
      setStatus('done');
    } catch (err) {
  ...
  return (
    <main className={styles.main}>
      <div className={styles.toolContainer}>
        ...
        <h1 className={styles.toolTitle}>JPG to PNG</h1>
        <p className={styles.toolDescription}>Convert JPG images to PNG locally in your browser.</p>
