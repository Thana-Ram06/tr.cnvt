...
export default function JpgToWebpPage() {
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (f?.type === 'image/jpeg') { setFile(f); setErrorMsg(''); } else if (f) setErrorMsg('Please select a JPG file.');
  }, []);

  const handleDrop = useCallback((e: React.DragEvent) => {
    e.preventDefault();
    const f = e.dataTransfer.files?.[0];
    if (f?.type === 'image/jpeg') { setFile(f); setErrorMsg(''); } else if (f) setErrorMsg('Please drop a JPG file.');
  }, []);
  ...
  return (
    <main className={styles.main}>
      <div className={styles.toolContainer}>
        ...
        <h1 className={styles.toolTitle}>JPG to WEBP</h1>
        <p className={styles.toolDescription}>Convert JPG to WEBP locally in your browser.</p>
        <div className={styles.uploadArea} onClick={() => inputRef.current?.click()} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && inputRef.current?.click()}>
          <input ref={inputRef} type="file" accept="image/jpeg" onChange={handleFileChange} className={styles.fileInput} aria-label="Select JPG" />
          <div className={styles.uploadVisual} aria-hidden />
          <p className={styles.uploadText}>{file ? file.name : 'Click to select JPG or drag and drop'}</p>
          <p className={styles.uploadHint}>JPG only</p>
        </div>
